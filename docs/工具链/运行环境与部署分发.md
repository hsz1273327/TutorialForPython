...
要分发包,首先要可以将我们的模块安装到python的第三方库(windows下默认是在python根目录的`Lib\site-packages`目录下).要实现这个功能我们需要用到`setuptools`.

与pip一样,`setuptools`是python的准标准包,他比pip出现更早,它的设计目的和pip一样也是为了分发python的第三方模块.`pip`兼容`setuptools`,它现在的功能更多的是用来实现一些特殊功能,就比如定义安装脚本.


### 用setuptools编写安装脚本

安装脚本`setup.py`就类似npm的`package.json`,它负责设定包的基本信息和依赖,以下是一个官方的例子
setup.py:
...
npm可以定义`scripts`字段通过命令行执行一些功能,我们的python看起来似乎并不具备这个能力,但其实不然,我们可以改造`setup.py`来实现一些简单的功能,比如:
...
if __name__ = '__main__':
    if sys.argv[-1] == 'publish':
        ...
        sys.exit()
```
在`setup.py`脚本最后添加一个执行命令,利用子进程来实现一些功能,本处实现了自动上传代码到pypi的功能.同理,我们也可把测试嵌进去
...
if __name__ = '__main__':
    if sys.argv[-1] == 'test':
        os.system("python -m unittest discover -v -s ./src")
        sys.exit()
```
...
完成pypi上的注册,并定义好安装脚本后我们就可以将自己写的模块上传到pypi服务器上了.
